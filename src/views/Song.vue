			...
			<div class="bg-white rounded border border-gray-200 relative flex flex-col">
				...
				<div class="p-6">
					...
					<vee-form :validation-schema="schema" @submit="addComment" v-if="userLoggedIn">
						...
						<button class="py-1.5 px-3 rounded text-white bg-green-600 block" type="submit"
...
export default {
	...
	async created() {
		const docSnapshot = await firebase.songsCollection.doc(this.$route.params.id).get();
	...
	methods: {
		...
		async addComment(values, { resetForm }) {
			...
			await firebase.commentsCollection.add(comment);

			this.song.comment_count += 1;
			await firebase.songsCollection.doc(this.$route.params.id).update({
		...
		async getComments() {
			const snapshots = await firebase.commentsCollection.where('sid', '==', this.$route.params.id).get();
